...
export function Checkbox({ label, className = '', ...rest }: CheckboxProps) {
  return (
    <label className={`inline-flex items-center gap-3 cursor-pointer text-sm select-none ${className}`}>
      ...
      <span className="w-7 h-7 flex-none inline-flex items-center justify-center rounded-md bg-idle text-ink shadow-neu transition-all duration-150 ease-in-out peer-checked:bg-green peer-checked:text-white peer-focus-visible:outline-2 peer-focus-visible:outline-green peer-focus-visible:outline-offset-2 [&>svg]:opacity-0 peer-checked:[&>svg]:opacity-100">
        <Check size={14} />
      </span>
