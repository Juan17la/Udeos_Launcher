...
/** Faint items floating behind a page: the theme's decor assets when set in
 *  assets/, else pixel icons. Parent must be position:relative + overflow:hidden.
...
export default memo(function Decor({ slots, opacity = 0.13, offset = 0 }: { slots: DecorSlot[]; opacity?: number; offset?: number }) {
  const { theme } = useApp()
  const images = ASSETS.decor[theme]
    ...
    <>
      {slots.map((s, i) => (
        <span key={i} className="absolute pointer-events-none z-0 [image-rendering:pixelated]" style={{ top: s.top, left: s.left, width: s.size, height: s.size, transform: `rotate(${s.rot}deg)`, opacity }}>
          {images.length > 0
            ? <img src={images[(i + offset) % images.length]} alt="" width={s.size} height={s.size} />
