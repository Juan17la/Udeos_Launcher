...
export default function DropZone({ text, children, className = '' }: { text: string; children?: ReactNode; className?: string }) {
  ...
  return (
    <div
      className={`flex items-center justify-center flex-wrap gap-4 p-4 rounded-md border-2 border-dashed text-sm text-muted transition-all duration-150 ease-in-out ${over ? 'border-green bg-green/10' : 'border-idle'} ${className}`}
