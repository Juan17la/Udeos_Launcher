  ...
  return (
    <div className="relative flex-none" ref={ref}>
      <button
        ...
        title={nickname}
        onClick={() => setOpen((v) => !v)}
        className="inline-flex items-center gap-2 cursor-pointer font-heading font-extrabold text-[15px] text-text bg-neutral-200 border border-divider rounded-full py-[8.8px] px-[15.84px] min-w-[120px] max-w-[200px] hover:bg-neutral-300"
      >
        <User size={16} />
            ...
            <div className="flex flex-col gap-0.5 max-h-40 overflow-y-auto pl-[8.8px]">
              {LANGUAGES.map((l) => (
                <button type="button" key={l.code} className={`${option} text-[13px]`} onClick={() => { setLanguage(l.code); setLangOpen(false) }}>
                  <span className="w-[14px] flex-none inline-flex text-accent">{language === l.code && <Check size={12} />}</span>
                  {l.name}
                </button>
              ))}
            </div>
